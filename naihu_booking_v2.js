...
function pushQueue(tpsite, date, time, stage) {
    var url = '../tp' + tpsite + '.aspx?module=net_booking&files=booking_place&StepFlag=25&QPid=' + stage + '&QTime=' + time + '&PT=1&D=' + year + '/' + date;
...
function lauchBooking() {
    ...
    return $.ajax({
        ...
        success: (e) => {
            ...
            console.log(`[場次預訂] ${dataset} 已經回應, :: [預定場地中，請耐心等候！]`);
            if (penddingAjax.length < maxWorker) {
                lauchBooking();
            }
        },
        error: (err) => {
            console.log('[場次預訂錯誤] err: ', dataset + String(err).substring(0, 60));
            penddingAjax.splice(0, 1);
...
function checkls(tpsite= '12', registerTimes=[15,16,17,18], instant=false) {
    ...
    const gapTime = isBefore || isAfter ? (instant ? 1:500) : idleSeconds * 1000;
...
function ajaxCheckList(tpsite = '12', date = '04/24', page = 2, allowedTimes = [15,16,17], gapTime) {
    window.setTimeout(() => {
        ...
        return $.ajax({
            url: `https://${domain}/tp${tpsite}.aspx?module=net_booking&files=booking_place&StepFlag=2&PT=1&D=${year}/${date}&D2=${page}`,
            method: 'GET',
            success: (cxt) => {
